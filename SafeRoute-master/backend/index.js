...
async function sendSmsethiopiaSms(to, body) {
  ...
  try {
    ...
    if (err.name === "AbortError") {
      throw new Error("SMSEthiopia request timed out after 10 seconds");
    }
    throw err;
  }
}
...
async function sendSms(to, body) {
  ...
  console.log("canSendSms =", canSendSms);
  ...
  try {
    ...
    throw new Error("SMS failed — check Twilio or SMSEthiopia setup");
  }
}
...
app.post("/api/test-sms", async (req, res) => {
  ...
  typeof phone_number === "string"
    ...
    console.log("SMS_PROVIDER:", SMS_PROVIDER);
    console.log("canSendSms:", canSendSms);
    console.log("SMSETHIOPIA_API_URL:", SMSETHIOPIA_API_URL);
    ...
    return res.json({ ok: true, phone_number: cleanPhone, sms: result });

  } catch (err) {
    console.error("SMS test error FULL:", err);
